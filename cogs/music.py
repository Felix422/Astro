...
class YTDLSource(discord.PCMVolumeTransformer):
	...
	async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
		...
		if data is None:
			raise commands.CommandError(f'Could not find anything that matches `{search}`')
		...
		if processed_info is None:
			raise commands.CommandError(f'Couldn\'t fetch {webpage_url}')
		...
		if 'entries' not in processed_info:
			...
			while info is None:
				try:
					...
					raise commands.CommandError(f'Couldn\'t retrieve any matches for `{webpage_url}`')
...
class Song:
	def __init__(self, source: YTDLSource, requester):
		self.source = source
...
class VoiceState:
	...
	def play_next_song(self, error=None):
		if error:
			raise self._ctx.send(str(error))
